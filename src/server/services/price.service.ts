...
type PollPriceSourceMap = Map<string, number>;
...
async function readOwnedPokemonCardIdBatch(cursor: string | null, limit: number): Promise<FetchOwnedPokemonCardIdBatchResult> {
...
async function readCatalogRowsByIds(pokemonCardIds: string[]): Promise<PokemonCardCatalogRow[]> {
  ...
  const result = await query<PokemonCardCatalogRow>(
    `SELECT pc.id, pc.tcg_id, pc.set_id, pc.rarity_tier, pc.current_price
...
async function readCatalogBatch(
  ...
  if (!cursor) {
    const fromStart = await query<PokemonCardCatalogRow>(
      `SELECT id, tcg_id, set_id, rarity_tier, current_price
       FROM pokemon_cards
...
async function resolvePriceSource(rows: PokemonCardCatalogRow[]): Promise<PollPriceSourceMap> {
  const priceByPokemonCardId = new Map<string, number>();
  ...
  for (const row of rows) {
    ...
    if (typeof externalPrice === "number" && externalPrice > 0) {
      priceByPokemonCardId.set(row.id, externalPrice);
      continue;
    }

    priceByPokemonCardId.set(row.id, calculateNextSimulatedPrice(row, setDriftBySetId));
  }
  ...
  return withTransaction(async (client) => {
    ...
    for (const candidate of candidates) {
      const result = await client.query<UpdatedPokemonCardRow>(
        ...
        [candidate.id, candidate.nextPrice]
      );
...
export async function pollPriceBatch(cursor: PollPriceCursor): Promise<PollPriceBatchResult> {
  ...
  for (const row of batchRows) {
    const currentPrice = Math.max(toMoneyCents(row.current_price), 1);
    const nextPrice = nextPriceByPokemonCardId.get(row.id);
    ...
    if (nextPrice !== currentPrice) {
      candidates.push({ id: row.id, nextPrice });
    }
  }

  const updatedRows = await updateCatalogPrices(candidates);
  await syncPriceCache(updatedRows);
