...
export default function AuctionsPage(): JSX.Element {
  ...
  const itemLabel = useMemo(() => {
    if (auctions.total === 1) {
      return "1 active auction";
    }
    return `${auctions.total} active auctions`;
  }, [auctions.total]);
    ...
    <section className="space-y-5">
                  ...
                  <article key={card.id} className="rounded-xl border border-slate-200 bg-slate-50 p-3">
                    ...
                    <button
                      ...
                      className={`mt-3 w-full rounded-xl px-3 py-2 text-sm font-bold transition ${
                        pending ? "cursor-not-allowed bg-slate-200 text-slate-500" : "bg-slate-900 text-white hover:bg-slate-700"
                      }`}
                    >
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {auctions.auctions.map((auction) => (
          <AuctionListingCard key={auction.id} auction={auction} currentUserId={user?.id ?? null} />
        ))}
      </div>
    </section>
  );
