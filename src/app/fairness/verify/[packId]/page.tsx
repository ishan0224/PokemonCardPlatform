    ...
    const run = async (): Promise<void> => {
      ...
      try {
        const [vectorResponse, packResponse] = await Promise.all([
          fetch("/api/fairness/verify-test-vector", { credentials: "include", cache: "no-store" }),
          fetch(`/api/fairness/pack/${params.packId}`, { credentials: "include", cache: "no-store" })
        ]);

        if (!vectorResponse.ok) {
          throw new Error(`Failed to load canonical vector (${vectorResponse.status}).`);
        }
        if (!packResponse.ok) {
          throw new Error(`Failed to load pack fairness data (${packResponse.status}).`);
        }

        const vectorPayload = (await vectorResponse.json()) as VectorApiResponse;
        const packPayload = (await packResponse.json()) as PackApiResponse;
