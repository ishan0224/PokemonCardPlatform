...
type DropsPageClientProps = {
  initialUpcoming: Drop[];
  initialLive: Drop[];
};

export function DropsPageClient({ initialUpcoming, initialLive }: DropsPageClientProps): JSX.Element {
  ...
  // Use server-fetched data until client hooks have loaded
  const upcomingDrops = upcoming.drops.length > 0 || !upcoming.loading ? upcoming.drops : initialUpcoming;
  const liveDrops = live.drops.length > 0 || !live.loading ? live.drops : initialLive;
  const isUpcomingLoading = upcoming.loading && upcomingDrops.length === 0;
  const isLiveLoading = live.loading && liveDrops.length === 0;

  const realtimeDropIds = useMemo(
    () => [...new Set([...upcomingDrops.map((d) => d.id), ...liveDrops.map((d) => d.id)])],
    [liveDrops, upcomingDrops]
  );
  ...
  const hero = pickHeroDrop(upcomingDrops, liveDrops);
  const showUpcoming = filter === "all" || filter === "upcoming";
  const showLive = filter === "all" || filter === "live";

  const totalResults = upcomingDrops.length + liveDrops.length;
        ...
        <section aria-labelledby="upcoming-drops-heading" className="space-y-4">
          <header className="flex items-end justify-between gap-3">
            ...
            <p className="text-[12px] text-pv-muted">
              {upcomingDrops.length} drop{upcomingDrops.length === 1 ? "" : "s"}
            </p>
          </header>
          ...
          {!isUpcomingLoading && !upcoming.error && upcomingDrops.length === 0 ? (
          ...
          {upcomingDrops.length > 0 ? (
            <PaginationFooter
              ...
              loadedCount={upcomingDrops.length}
            />
          ) : null}
        </section>
      ) : null}
      ...
      {showLive ? (
        <section aria-labelledby="live-drops-heading" className="space-y-4">
          <header className="flex items-end justify-between gap-3">
            ...
            <p className="text-[12px] text-pv-muted">
              {liveDrops.length} drop{liveDrops.length === 1 ? "" : "s"}
            </p>
          </header>
          ...
          {!isLiveLoading && !live.error && liveDrops.length === 0 ? (
            <div className="rounded-pv-lg border border-pv-line bg-pv-surface-2 p-5 text-[13px] text-pv-muted">
